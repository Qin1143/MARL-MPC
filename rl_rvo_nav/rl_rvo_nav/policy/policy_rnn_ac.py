import torch # torch 是 PyTorch 的核心库，用于张量操作和自动微分
import torch.nn as nn # torch.nn 是 PyTorch 的神经网络模块，包含了构建和训练神经网络所需的各种类和函数
import numpy as np
from gym.spaces import Box # Box 是 OpenAI Gym 中的一个类，用于定义连续动作空间或状态空间
from torch.distributions.normal import Normal # Normal 是 PyTorch 中的一个类，用于创建和操作正态分布（高斯分布）
from torch.nn.utils.rnn import pad_sequence, pack_padded_sequence # pad_sequence 用于将一系列不同长度的序列填充到相同长度，pack_padded_sequence 用于将填充后的序列打包，以便在 RNN 中高效处理变长序列
import time
...
class rnn_ac(nn.Module): # rnn_ac 类将拥有 nn.Module 的所有属性和方法（nn.Module：所有神经网络模块的基类）
